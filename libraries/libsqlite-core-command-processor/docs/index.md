...
(compiles expressions to bytecode), and `sqlite::core::virtual_machine`
(runs it). `sqlite::utils` is test-harness-only, as everywhere else.
...
Schema is a caller-supplied **in-memory** `table_descriptor` (name, ordered
column list, `std::vector<std::vector<aux::mem>>` rows) -- an explicit,
deliberate stand-in for a real catalog backed by `sqlite-backend-tree`, the
same kind of substitution `code_generator`'s resolver callbacks already
are for a real `Parse` context. Wiring this up to real B-tree-backed
storage is real, substantial follow-up work: it needs an on-disk
row-record (de)serialization format (legacy `record.c`/`vdbeaux.c`'s
`sqlite3VdbeRecordUnpack`), which doesn't exist anywhere in this workspace
yet -- not attempted this pass.
